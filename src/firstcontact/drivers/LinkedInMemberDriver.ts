/**
 * LinkedIn OAuth2 driver for FirstContact — the older member API.
 *
 * Kept for applications whose LinkedIn app still has `r_liteprofile` /
 * `r_emailaddress` rather than OpenID Connect. It needs two calls: the
 * profile carries no address, and the address endpoint carries no profile.
 *
 * New applications want {@link LinkedInDriver}.
 */
...
export class LinkedInMemberDriver extends Oauth2Driver {
